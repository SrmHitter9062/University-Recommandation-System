...
<h1>Technical News (courtsety: Times Of India )</h1>
<br />
<?php
$cur = date('d-m-Y H:i:s');
$cur = strtotime($cur);
mysql_query("UPDATE user_behaviour SET technical= technical+1
WHERE username= '$name'");
$result = mysql_query("SELECT title , link,date FROM  `news_story` WHERE type_news ='technical' ORDER BY  `news_story`.`date` DESC ");
